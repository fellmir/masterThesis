...
if (!fs::dir_exists("outputs/RP/dynamic/1st")) fs::dir_create("outputs/RP/dynamic/1st")
if (!fs::dir_exists("outputs/RP/dynamic/2nd")) fs::dir_create("outputs/RP/dynamic/2nd")
...
purrr::walk(
  ...
  function(i) {
    ...
    data <- fs::dir_ls("db/", regexp = "rais(.*)\\.parquet") %>%
      ...
      dplyr::mutate(
        rel_year = ifelse(
          treat_RP == 1, (ano-2003), Inf
        )
      ) %>%
      dplyr::mutate(log_avg_income_defl = log(avg_income_defl)) %>%
      dplyr::select(-c(avg_income_defl, treat_RP, post, idade)) %>%
      dplyr::collect() %>%
      dplyr::group_by(ano) %>%
    ...
    purrr::walk(
      ...
      function(j) {
        if (length(i) == 1) {
          ...
          name_file_1st <- glue::glue("outputs/RP/dynamic/1st/dynamic_1st_{j}_in_{i}_RP.rds")
          
          name_file_2nd <- glue::glue("outputs/RP/dynamic/2nd/dynamic_2nd_{j}_in_{i}_RP.rds")
        } else {
          i[3] <- "under_40"
          ...
          name_file_1st <- glue::glue("outputs/RP/dynamic/1st/dynamic_1st_{j}_in_{janitor::make_clean_names(k)}_RP.rds")
          
          name_file_2nd <- glue::glue("outputs/RP/dynamic/2nd/dynamic_2nd_{j}_in_{janitor::make_clean_names(k)}_RP.rds")
        }
...
purrr::walk(
  ...
  function(i) {
    ...
    data <- fs::dir_ls("db/", regexp = "rais(.*)\\.parquet") %>%
      ...
      # dplyr::mutate(log_avg_income_defl = log(avg_income_defl)) %>%
      dplyr::select(-c(treat_RP, post, idade)) %>%
      dplyr::collect() %>%
      dplyr::group_by(ano) %>%
      # dplyr::slice_sample(prop = 0.1) %>%
      data.table::as.data.table()
