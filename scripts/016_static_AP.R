...
if (!fs::dir_exists("outputs/AP/static")) fs::dir_create("outputs/AP/static")
...
purrr::walk(
  ...
  function(i) {
    ...
    data <- fs::dir_ls("db/", regexp = "rais(.*)\\.parquet") %>%
      ...
      dplyr::select(-c(avg_income_defl, treat_AP)) %>%
      dplyr::collect() %>%
      dplyr::group_by(ano) %>%
    ...
    purrr::walk(
      ...
      function(j) {
        if (length(i) == 1) {
          ...
          name_file <- glue::glue("outputs/AP/static/static_{j}_in_{i}_AP.rds")
        } else {
          k <- stringr::str_flatten(i, collapse = " + ")
          
          first_stage_fml <- glue::glue("{j} ~ 0 + {k} | {fixed_vars[1]} + {fixed_vars[2]}")
          
          name_file <- glue::glue("outputs/AP/static/static_{j}_in_{janitor::make_clean_names(k)}_AP.rds")
        }
